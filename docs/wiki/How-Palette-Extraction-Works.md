...
Same picture in, same palette out, on any machine.
...
Counts run 1–20, minor from 0, default 6 each; too few distinct colours is
reported, never padded. The seed is an FNV-1a hash of the downscaled pixels plus
a per-pass salt — no clock, no default `Random()`.
...
`Major 01`… and `Minor 01`…. Files land in
`%LOCALAPPDATA%\CSP Palette Companion\Palettes`; code and tests in
